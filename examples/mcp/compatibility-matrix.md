...
| Anthropic Claude Messages MCP | Source-aligned template prepared | `examples/mcp/anthropic-messages-mcp.mjs` follows the Claude Messages MCP connector shape with `mcp_servers`, `authorization_token`, `mcp_toolset`, five enabled Neura tool configs, and `mcp-client-2025-11-20`; `npm run example:anthropic-mcp-request` prints a sanitized request for review; live Claude client verification is pending until `ANTHROPIC_API_KEY` is available |
