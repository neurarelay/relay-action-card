...
| --- | --- | --- |
| Neura Relay MCP server | Production verified | Protected `/mcp`, initialize, tool-scoped `tools/list` / `tools/call`, auth rejection, validation, resolution, receipt lookup, trace replay, Agent Passport lookup with `authority_standing`, no private payload leakage |
