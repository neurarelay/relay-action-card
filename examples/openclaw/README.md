...
The `preflight-adapter` folder adds the stable package for local autonomous computer-use runtimes: `@neurarelay/openclaw-preflight-adapter@0.1.5` under npm and ClawHub `latest,stable` tags. The artifact scan is clean. The old npm `rc` tag remains available only for release-candidate history. This is not an official OpenClaw / ClawHub listing, approval, endorsement, integration, or partnership claim.
