...
The `preflight-adapter` folder adds the npm-published `beforeAction(preflightAction)` release candidate for local autonomous computer-use runtimes: `@neurarelay/openclaw-preflight-adapter@0.1.0-rc.1` under the `rc` tag. It is not submitted, listed, approved, or partnered by OpenClaw / ClawHub.
